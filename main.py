...
def everyday():
    if (everyday_API!="否"):
        conn = http.client.HTTPSConnection('api.tianapi.com')  #接口域名
        params = urllib.parse.urlencode({'key':everyday_API})
        headers = {'Content-type':'application/x-www-form-urlencoded'}
...
#推送信息
def send_message(to_user, access_token, city_name, weather, max_temperature, min_temperature, pipi, lizhi, pop, tips, note_en, note_ch, health_tip, lucky_, everyday, pyq):
...
if __name__ == "__main__":
    ...
    #获取每日英文API
    everyday_API=config["everyday_API"]
    #获取朋友圈API
    ...
    # 公众号推送消息
    for user in users:
        send_message(user, accessToken, city, weather, max_temperature, min_temperature, pipi, lizhi,pop,tips, note_en, note_ch, health_tip, lucky_, everyday, pyq)
    import time
    time_duration = 10
